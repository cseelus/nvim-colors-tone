...
-- Based On:   https://github.com/NTBBloodbath/doom-one.nvim
...
-- Set default values for tone variables if they don't exists
if vim.g.tone_enable_treesitter == nil then
	vim.g.tone_enable_treesitter = true
end
if vim.g.tone_transparent_background == nil then
	vim.g.tone_transparent_background = false
end
if vim.g.tone_italic_comments == nil then
	vim.g.tone_italic_comments = false
end
if vim.g.tone_telescope_highlights == nil then
	vim.g.tone_telescope_highlights = true
end

local transparent_bg = vim.g.tone_transparent_background
...
local yellow = '#ebff00'
local water = '#6cbeff'
local dark_water = '#2257A0'
local sky = '#bdbde7'
local light_sky = utils.Lighten(sky, 0.4)
local aubergine = '#c59dc5'
local cyan = '#46D9FF'
local white = '#efefef'
...
local tag = utils.Mix(water, cyan, 0.5)
...
if current_bg == 'light' then
	...
	dark_water = '#a0bcf8'
	sky = '#a626a4'
	light_sky = utils.Darken(sky, 0.36)
	aubergine = '#b751b6'
	cyan = '#0184bc'
	white = '#efefef'
